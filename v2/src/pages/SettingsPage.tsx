    ...
    <div className="mx-auto max-w-2xl p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <header className="border-b border-white/5 pb-4">
        ...
        <p className="mt-1 text-sm text-zinc-400">
          {user ? '☁️ Cloud Sync Active (เข้าสู่ระบบแล้ว)' : '💾 Local Storage Mode (Guest Mode)'}
        </p>
      </header>
      ...
      <section className="lg-surface p-5 space-y-4">
        ...
        <div className="lg-content space-y-4">
          <h2 className="text-sm font-bold text-zinc-200 uppercase tracking-wider">โปรไฟล์และการเชื่อมโยงข้อมูล</h2>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full bg-white/[0.03] border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-violet-500"
              />
            </div>
            
            <div className="space-y-1">
              <label className="text-xs font-semibold text-zinc-400">อีเมลลงทะเบียน (Email)</label>
              <input
                type="text"
                disabled
                value={user?.email || 'Guest User (ไม่มีการเชื่อมโยงอีเมล)'}
                className="w-full bg-white/[0.01] border border-white/5 rounded-xl px-4 py-2.5 text-xs text-zinc-500 select-none"
              />
            </div>
          </div>
        </div>
      ...
      <section className="lg-surface p-5 space-y-4">
        ...
        <div className="lg-caustic" />
        <div className="lg-content space-y-4">
          <h2 className="text-sm font-bold text-zinc-200 uppercase tracking-wider">การปรับแต่งแล็บ (Lab Preferences)</h2>
          ...
          <div className="space-y-3">
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full bg-zinc-900 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-violet-500"
              >
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-zinc-400">สีย้อมแกรมเริ่มต้น (Default Gram Stain)</label>
              <select
                value={defaultGram}
                onChange={(e) => setDefaultGram(e.target.value)}
                className="w-full bg-zinc-900 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-violet-500"
              >
                <option value="">ไม่มีสีย้อมแกรมเริ่มต้น (เริ่มด้วยการวิเคราะห์ใหม่ทุกครั้ง)</option>
            ...
            <div className="flex items-center justify-between p-3 rounded-xl border border-white/5 bg-white/[0.01]">
              <div>
                <label className="text-xs font-bold text-zinc-200 block">บันทึกเคสอัตโนมัติ (Auto-Save Cases)</label>
                <span className="text-[10px] text-zinc-500">บันทึกสถิติและเคสลงประวัติหลังจากวินิจฉัยเสร็จสิ้น</span>
              </div>
              <input
                type="checkbox"
                checked={autoSave}
      ...
      <div className="flex items-center justify-between gap-4">
        <button
          type="button"
          onClick={handleReset}
          className="px-4 py-2.5 rounded-xl border border-white/10 text-xs font-semibold text-zinc-400 hover:bg-white/5 transition"
        >
        ...
        <button
          ...
          disabled={saving}
          onClick={handleSave}
          className="px-6 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-r from-violet-600 to-indigo-600 text-white shadow-md shadow-violet-500/20 hover:scale-[1.02] active:scale-[0.98] transition disabled:opacity-50"
        >
